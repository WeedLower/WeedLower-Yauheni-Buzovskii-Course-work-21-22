...
public interface ItemsRepository extends JpaRepository<ItemsEntity,Integer> {
    ...
    @Query(value = "select new com.example.backend.model.ItemModel(i,count (a.id.userId))" +
            " from ItemsEntity i left join ItemsLikes a on i.id=a.id.itemId " +
            "where i.id= :id group by i")
    Optional<ItemModel> findItemById(@Param("id") Integer id);

    @Query(value = "select new com.example.backend.model.ItemModel(i,count (a.id.userId)," +
            "sum (case when a.id.userId = :userId then 1 else 0 end ) > 0) from ItemsEntity i " +
            "left join ItemsLikes a on i.id=a.id.itemId where i.id = :id group by i")
    Optional<ItemModel> findItemAuthUser(@Param("id") Integer id, @Param("userId") Integer userId);

    @Transactional
    @Modifying
    @Query(value = "insert into postgres_db.items_likes(item_id, user_id) " +
            "VALUES (:itemId,:userId)", nativeQuery = true)
    void like(@Param("itemId") Integer id, @Param("userId") Integer id1);

    @Query(value = "select i from ItemsEntity i left join ItemsTag t on i.id=t.id.itemId " +
            "where t.id.tagId= :id group by i")
